...
> 이때부터 **'파일(File)'**이라는 개념이 표준화, 텍스트 에디터로 코딩하기 시작
...
- **왜 전송 중에 데이터가 망가질까? (원자성 부족)**
	- **이유: "한 번에 다 저장하지 않고, 하나씩 순서대로 저장하기 때문입니다."**
	- 이걸 전문 용어로 **'원자성(Atomicity)이 없다'** 고 합니다.
	  
	- 우리가 기능을 하나 수정할 때 보통 파일 여러 개를 동시에 고칩니다. 예를 들어, `기능.c`와 `설정.h` 두 개를 고쳐서 "로그인 기능 추가"를 완성했다고 해봅시다. 이 두 파일은 **한 몸처럼(Set)** 움직여야 합니다.
	  
	- CVS의 저장 방식 (순차 처리)
	- 개발자가 "저장(Commit)!" 버튼을 누르면, CVS 서버는 내부적으로 이렇게 작동합니다.
		1. `기능.c` 저장 시작... **성공!** (서버: v2로 업데이트됨)
		2. `설정.h` 저장 시작...
		- _(이때 갑자기 정전이 되거나 인터넷 선이 뽑힘!)_
		- **실패!** (서버: 여전히 v1임)
	- **대참사 결과** 
		- 서버에는 **`기능.c`는 최신(v2)** 인데, 짝꿍인 **`설정.h`는 옛날 거(v1)** 가 남아있습니다.
	- 이 상태에서 다른 개발자가 코드를 받으면? -> **짝이 안 맞아서 프로그램이 실행조차 안 됩니다.** (컴파일 에러)
	- 더 무서운 건, **"어디까지 저장되다 말았는지"** CVS도 모릅니다.

- **전역 리비전 (Global Revision)**
	- "파일마다 번호를 매기지 말고, 프로젝트 전체에 번호를 매기자."
	1.  **CVS의 문제점: "제각각인 시계"**
		CVS는 파일마다 버전 번호가 따로 놀았습니다.
		- `main.c`: v1.4
		- `login.c`: v1.9
		- `design.css`: v1.1
		
		이러면 "야, 우리 지난주 금요일에 배포했던 버전(특정 시점)으로 되돌리자!" 라고 했을 때 대혼란이 옵니다.
		
		"어... 그때 `main.c`는 1.2였나? `login.c`는 1.5였나?"
		
		이것을 맞추기 위해 개발자들은 파일마다 일일이 날짜 태그를 붙여야 하는 노가다를 했습니다.
		
	1. **SVN의 해결책: "프로젝트 전체 스냅샷 번호"**
	 - SVN은 누군가 커밋(저장)을 할 때마다, 파일 하나만 바뀌었더라도 **저장소 전체의 번호(리비전)** 를 1씩 올립니다. 이를 **전역 리비전** 이라고 합니다.
		 - **Revision 0:** 프로젝트 생성 (빈 깡통)
		 - **Revision 1:** 파일 100개 최초 등록 (`main.c`도 Rev 1, `login.c`도 Rev 1)
		 - **Revision 2:** 철수가 `main.c` **한 줄만** 수정해서 올림.
		 - **결과:** 저장소 전체가 **Revision 2**가 됩니다.
		 - `main.c`: 수정된 내용 (Rev 2)
		 - `login.c`: 안 바꼈지만, Rev 2 시점의 프로젝트 구성원.
	
	1. 왜 좋은가? (효용)
		- 이제 시점을 이야기하기가 너무 쉬워집니다.
		- **"리비전 100번으로 돌아가자."**
	    - 이 말은 **"리비전 100번 커밋이 일어났던 그 순간의 프로젝트 전체 상태(스냅샷)를 가져와"** 라는 뜻과 완벽하게 일치합니다.
		- 게임의 **'세이브 파일'** 과 같습니다. "Save 1", "Save 2"가 있으면 그 시점의
		- 
		- 내 캐릭터, 아이템, 맵 상태가 통째로 저장되는 것과 같은 원리입니다.

- **원자적 커밋 (Atomic Commit)**
	- **"전부 다 저장하든지, 아니면 아예 없던 일로 하든지. (All or Nothing)"**
	- '원자(Atom)'라는 말은 '더 이상 쪼갤 수 없다'는 뜻입니다. 즉, **커밋이라는 행위는 쪼개질 수 없는 하나의 덩어리**여야 한다는 원칙입니다.
	  
	 1. **CVS의 문제점: "쪼개지는 비극"**
		 - CVS는 파일 10개를 수정해서 올리면, 서버가 이걸 파일별로 하나씩 순서대로 저장했습니다.
			- 파일 1 저장 성공...
			- 파일 2 저장 성공...
			- **(와장창!) 인터넷 끊김 or 서버 정전**
			- 파일 3~10 저장 실패.
		- **결과:** 서버에는 파일 1, 2는 최신 버전(v2)인데, 나머지 3~10은 옛날 버전(v1)인 상태로 남습니다. 이걸 다른 개발자가 다운받으면? **소스 코드의 짝이 안 맞아서(Inconsistent) 프로그램이 실행되지 않습니다.** 서버가 '오염'된 것입니다.
	
	2.  **SVN의 해결책: "트랜잭션(Transaction)"**

SVN은 데이터베이스의 **트랜잭션 기술**을 도입했습니다. 과정은 다음과 같습니다.

1. **준비 (Journaling):** 사용자가 보낸 파일 10개를 바로 덮어쓰지 않고, **임시 공간**에 일단 다 받아서 쌓아둡니다.
    
2. **검증:** 10개가 문제없이 다 도착했는지 확인합니다.
    
3. **결정의 순간:**
    
    - 만약 중간에 끊겼다? -> 임시 공간에 있는 걸 **싹 폐기합니다(Rollback).** 리비전 번호는 올라가지 않습니다. (아무 일도 없던 것처럼 됨)
        
    - 완벽하게 다 있다? -> **"스위치 ON!"** 아주 짧은 순간에 임시 공간의 데이터를 실제 저장소로 반영하고 리비전 번호를 1 올립니다.
        

#### (3) 왜 좋은가? (효용)

- **절대적 안전:** 서버가 불타지 않는 이상, 저장소에 있는 데이터는 **항상 완벽한 세트(Consistent State)**임이 보장됩니다.
    
- "빌드가 깨졌다"는 상황이 줄어듭니다. 개발자들은 **"저장하다 말아서 생긴 오류"**를 걱정할 필요가 없어졌습니다.